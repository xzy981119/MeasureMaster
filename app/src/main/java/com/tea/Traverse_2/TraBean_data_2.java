...
public class TraBean_data_2 {
    ...
    //分别为起始点和结束点的坐标
    public static TraBean_data_2.ZuoBiao B;
    ...
    //平差结果,即各点坐标
    public static ArrayList<TraBean_data_2.ZuoBiao> resultList = new ArrayList<>();
    ...
    public static void jisuan(){
        ...
        bhc = k * sum_jiao - k * (guanCeJList.size()) * Math.PI + n;
        if (Geopro.hudutos(bhc) > 24 * Math.sqrt(guanCeJList.size()))
        {
            warn_jiao = true;
        }
        else
        ...
        {
            Xzengliang.add(disList.get(i) * Math.cos(fangweijiao.get(i )));
            Yzengliang.add(disList.get(i) * Math.sin(fangweijiao.get(i )));
        }
        sum_X = 0;
        ...
        for (int i = 0; i < disList.size(); i++)
        {
            resultList.add(new TraBean_data_2.ZuoBiao(resultList.get(i).getX() + Xzengliang.get(i) - xbhc * disList.get(i) / sum_dis,
